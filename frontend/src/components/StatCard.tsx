...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <div className={`glass-panel p-5 border-l-4 ${style.border} ${style.glow} transition-all duration-200 group relative overflow-hidden`}>
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{title}</span>
        <div className={`p-2.5 rounded-xl border ${style.bgIcon} transition-transform group-hover:scale-105`}>
          {icon}
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-xs pt-2 border-t border-slate-800/60">
        ...
        {trendPercentage && (
          <span className={`font-mono text-[11px] font-semibold ${trendPercentage.startsWith('+') ? 'text-emerald-400' : 'text-rose-400'}`}>
            {trendPercentage}
          </span>
        )}
      </div>
