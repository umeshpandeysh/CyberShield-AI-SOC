...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header
      role="banner"
      className={`h-16 fixed top-0 right-0 z-30 bg-[#0b1120]/90 backdrop-blur-2xl border-b border-slate-800/80 px-6 flex items-center justify-between transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center space-x-4 flex-1 max-w-md">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full bg-slate-900/90 border border-slate-800 rounded-xl pl-10 pr-12 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500/80 focus:ring-1 focus:ring-blue-500/40 transition"
          />
          <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700">
            /
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-5">
        ...
        <div className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-slate-900/90 border border-slate-800 text-xs">
          ...
          <span className="font-mono font-semibold text-slate-300 text-[11px] tracking-wide">
            {wsStatus === 'connected' ? 'LIVE TELEMETRY' : 'DISCONNECTED'}
          </span>
        </div>
        ...
        <div className="flex items-center space-x-3 border-l border-slate-800/80 pl-5">
          ...
          <button
            onClick={onLogout}
            className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition border border-transparent hover:border-rose-500/20 focus-visible:ring-2 focus-visible:ring-rose-500"
            title="Sign Out"
            aria-label="Sign Out"
          >
